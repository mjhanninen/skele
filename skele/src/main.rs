...
fn ask(prompt: &str, echo: bool) -> String {
  ...
  if echo {
    ...
    let result = rpassword::read_password()
      ...
      .unwrap();
    if !result.is_empty() {
      // `rpassword` echoes the newline character. Therefore we need to do a bit
      // of trickery in order to display the placeholder for entered text.
      t.cursor_up().unwrap();
...
fn ask_skeleton_key(state: &mut app_st::State) -> Option<String> {
  loop {
    ...
    let confirmation =
      ask("Please re-enter the skeleton key to confirm", false);
    if key == confirmation {
      return Some(key);
    }
